...
setInterval(()=>{
  ...
  .then(res=>{
      // 接口数据
      html=`<tr><td>${moment().format('HH:mm:ss')}</td><td>${res.data.list.marketName}</td><td>${res.data.list.flow}</td></tr>`
    ...
    axios('https://mock.mengxuegu.com/mock/609541ecc7b7385be0a82b0f/daping/24data?lg=31').then(res=>{
      options.series[0].data=res.data.dataMonth 
...
axios('https://mock.mengxuegu.com/mock/609541ecc7b7385be0a82b0f/daping/getRank').then(res=>{
  ...
  flowOptions.series[0].data=newArr
